...
class BoardRenderer:
    ...
    known here. Per-piece pixel position and (sprite_state, frame_index)
    are supplied by the caller via `visual_states` (keyed by piece_id); a
    piece absent from that mapping falls back to its resting cell position
    and the static idle sprite, exactly as in Stage 2/3.
    """

    def __init__(self, sprite_loader: SpriteLoader, board_image_path: Path, cell_size_px: int):
        self._sprite_loader = sprite_loader
        self._board_image_path = Path(board_image_path)
        self._cell_size_px = cell_size_px

    def render(self, board_snapshot, visual_states: Optional[Dict[int, PieceVisualState]] = None) -> Img:
        rows, cols = board_snapshot.rows, board_snapshot.cols
        visual_states = visual_states or {}

        frame = Img().read(self._board_image_path, size=(cols * self._cell_size_px, rows * self._cell_size_px))
        ...
        for piece_view in board_snapshot.pieces():
            ...
            sprite.draw_on(frame, pixel_x, pixel_y)
