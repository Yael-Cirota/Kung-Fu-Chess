...
def make_updater(elos=None, k_factor=32):
    ...
    updater = RatingUpdater(bus, users, game_records, EloCalculator(k_factor=k_factor))
    return bus, users, game_records, updater


def seat_fields(room_id="room-1", white_id=WHITE_ID, black_id=BLACK_ID, ended_at_ms=5000):
    """Every GAME_OVER payload carries these, attached by GameRoom at publish
    time - see game_room.py's `_settlement_fields`."""
    return {"room_id": room_id, "white_id": white_id, "black_id": black_id, "ended_at_ms": ended_at_ms}


def resign_event(resigned_color="white", **seat_kwargs):
    return Event(
        name=EventNames.GAME_OVER,
        payload={"reason": "disconnect", "resigned_color": resigned_color, **seat_fields(**seat_kwargs)},
    )


def server_fault_event(**seat_kwargs):
    return Event(
        name=EventNames.GAME_OVER,
        payload={"reason": "server_fault", **seat_fields(**seat_kwargs)},
    )


def engine_over_event(winner="white", at_ms=7000, **seat_kwargs):
    ...
    return Event(
        name=EventNames.GAME_OVER,
        payload={"engine_event": engine_event, **seat_fields(**seat_kwargs)},
    )


class TestSubscription:
    def test_constructing_it_is_enough_to_settle_a_game_over(self):
        bus, _users, records, _u = make_updater()
...
class TestForcedResignIsRated:
    def test_the_disconnected_player_loses_elo_and_the_opponent_gains(self):
        bus, users, _records, _u = make_updater()
    ...
    def test_records_the_game_with_the_disconnect_reason(self):
        ...
        publish(bus, resign_event(resigned_color="white", ended_at_ms=4321))
    ...
    def test_rating_change_reflects_the_gap_between_players(self):
        bus, users, _records, _u = make_updater(elos={WHITE_ID: 1600, BLACK_ID: 1200})
...
class TestEngineGameOverIsRated:
    def test_king_capture_rates_the_beneficiary_as_winner(self):
        bus, users, records, _u = make_updater()
...
class TestServerFaultIsAnUnratedDraw:
    def test_abandon_settles_as_a_draw_with_no_elo_change(self):
        bus, users, records, _u = make_updater()

        publish(bus, server_fault_event())

        assert users.updates == []
        assert records.records[0]["winner_id"] is None
        assert records.records[0]["reason"] == "server_fault"
...
class TestSettlementIsIdempotent:
    ...
    def test_different_rooms_settle_independently(self):
        ...
        bus = InMemoryEventBus()
        RatingUpdater(bus, users, records, EloCalculator())
...
class TestDbWriterRouting:
    def test_persistence_calls_go_through_the_db_writer_not_directly(self):
        ...
        RatingUpdater(bus, users, game_records, EloCalculator(), db_writer=db_writer)
...
class TestUnratedSituations:
    def test_a_room_missing_a_seated_player_is_not_settled(self):
        ...
        publish(bus, resign_event(resigned_color="white", black_id=None))

        assert users.updates == []
        assert records.records == []

    def test_a_missing_user_record_skips_the_elo_write_but_still_records(self):
        bus, users, records, _u = make_updater(elos={WHITE_ID: 1200})
    ...
    def test_an_unrecognised_resigned_color_leaves_the_game_unrated(self):
        ...
        publish(bus, Event(name=EventNames.GAME_OVER, payload={"resigned_color": "green", **seat_fields()}))
